...
while [ $i -le 5 ]; do
    ...
    # 溫度：scale=2.5177，結果單位 milli-degC，取到小數點後兩位
    # (raw + offset) * 25177 / 10000 = milli-degC
    T_MDEG=$(( (T_RAW + T_OFF) * 25177 / 10000 ))
    T_INT=$(( T_MDEG / 1000 ))
    T_FRAC=$(( (T_MDEG % 1000) / 10 ))
    if [ $T_FRAC -lt 0 ]; then T_FRAC=$(( -T_FRAC )); fi

    # 濕度：scale=1.5259，結果單位 milli-%，取到小數點後兩位
    # raw * 15259 / 10000 = milli-%
    H_MDEG=$(( H_RAW * 15259 / 10000 ))
    H_INT=$(( H_MDEG / 1000 ))
    H_FRAC=$(( (H_MDEG % 1000) / 10 ))

    printf "    Sample %d: Temp: %d.%02d C  Humidity: %d.%02d %%\n" \
