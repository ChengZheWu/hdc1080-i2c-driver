...
四個層次的嵌入式 I2C 驅動實做，全部跑在 QEMU 模擬環境，使用模擬感測器數據。
...
└── yocto/
    └── meta-hdc1080/               # 自訂 Yocto layer（需 commit）
        ├── conf/
        │   └── layer.conf          # layer 基本設定
        └── recipes-kernel/
            ├── hdc1080/
            │   ├── files/
            │   │   ├── hdc1080.c   # HDC1080 kernel module 源碼
            │   │   └── Makefile    # Kbuild Makefile
            │   └── hdc1080.bb      # BitBake recipe
            └── linux/
                ├── files/
                │   └── hdc1080.cfg # kernel config fragment（開啟 IIO）
                └── linux-yocto_%.bbappend  # 套用 config fragment
```
...
### Phase 1：Bare-metal（MPS2-AN385 / Cortex-M3）
...
### Phase 2：FreeRTOS（MPS2-AN385 / Cortex-M3）
...
### Phase 3：Linux Kernel Module（AST2600 / Cortex-A7 + Buildroot）
...
### Phase 4：Linux Kernel Module（Yocto + qemuarm）

使用 Yocto Project 建置系統，展示業界標準的嵌入式 Linux 開發流程。
建立自訂 `meta-hdc1080` layer，透過 BitBake recipe 自動編譯、打包 hdc1080.ko 進 rootfs，開機自動載入。

> **注意**：第一次需要完整建置，約 1-2 小時。
...
git clone --depth 1 -b scarthgap https://github.com/openembedded/meta-openembedded meta-openembedded
```
...
BB_NUMBER_THREADS = "8"
PARALLEL_MAKE = "-j 8"
DL_DIR = "/workspace/yocto/downloads"
SSTATE_DIR = "/workspace/yocto/sstate-cache"
EXTRA_IMAGE_FEATURES ?= "debug-tweaks"
LICENSE_FLAGS_ACCEPTED = "commercial"
IMAGE_INSTALL:append = " hdc1080"
...
qemu-system-arm \
    -machine virt \
    -cpu cortex-a15 \
    -kernel /workspace/yocto/build/tmp/deploy/images/qemuarm/zImage \
    -initrd /workspace/yocto/build/tmp/deploy/images/qemuarm/core-image-minimal-qemuarm.rootfs.cpio.gz \
...
登入（帳號 `root`，無密碼）後確認 driver 狀態：

```bash
lsmod | grep hdc1080
find /lib/modules -name "hdc1080.ko"
cat /etc/modules-load.d/hdc1080.conf
```
...
# find /lib/modules -name "hdc1080.ko"
/lib/modules/6.6.123-yocto-standard/extra/hdc1080.ko

# cat /etc/modules-load.d/hdc1080.conf
hdc1080
```

> **Note**：qemuarm 的 I2C controller 模擬不完整，driver 可以載入但無法完整 probe。
> 完整的 probe 和 IIO sysfs 驗證請參考 Phase 3（Buildroot + AST2600）。
...
### Phase 3：Linux Kernel（Buildroot）
...
### Phase 4：Linux Kernel（Yocto）

| 項目 | 說明 |
|------|------|
| 建置系統 | Yocto Project / BitBake，業界大型嵌入式 Linux 產品標準 |
| Layer 架構 | `meta-hdc1080` 自訂 layer，BSP / Distro / App 分層管理 |
| Recipe（.bb） | 描述如何下載、編譯、安裝 hdc1080.ko，取代手動 make module |
| Config Fragment | `.cfg` 檔案開啟 `CONFIG_IIO=y`，不需要互動式 menuconfig |
| bbappend | 擴充 kernel recipe 加入 config fragment，不修改上游 layer |
| 自動載入 | `KERNEL_MODULE_AUTOLOAD` 產生 `/etc/modules-load.d/hdc1080.conf` |
| 套件管理 | `IMAGE_INSTALL:append` 將 hdc1080 package 打包進 rootfs image |
| Manifest | image 產出 `.manifest` 檔，清楚列出所有安裝的 package 和版本 |

### Buildroot vs Yocto 對比

| 面向 | Buildroot（Phase 3） | Yocto（Phase 4） |
|------|---------------------|-----------------|
| 設定方式 | 單一 defconfig + .config | bblayers.conf + local.conf |
| 硬體設定 | 內建在 defconfig | 獨立 BSP layer（machine.conf）|
| Driver 整合 | 手動 make module + overlay | .bb recipe 自動化 |
| Kernel config | make menuconfig | .cfg fragment |
| DTS 修改 | 手動 git apply patch | .bbappend 自動套用 |
| 套件管理 | 無（整包 rootfs） | 有（rpm/deb/ipk）|
| 適用場景 | 小型專案、快速開發 | 大型商業產品、多平台維護 |
...
| Phase | QEMU Machine | SoC | 架構 |
|-------|-------------|-----|------|
| 1. Bare-metal | `mps2-an385` | ARM MPS2 | Cortex-M3 |
| 2. FreeRTOS | `mps2-an385` | ARM MPS2 | Cortex-M3 |
| 3. Linux（Buildroot） | `ast2600-evb` | ASPEED AST2600 | Cortex-A7 (dual core) |
| 4. Linux（Yocto） | `virt` | QEMU virt | Cortex-A15 |
...
- 加入 interrupt 驅動模式（取代 I2C polling）
- 加入 IIO triggered buffer，支援連續高速取樣
- 移植到真實硬體（Raspberry Pi i2c-1 + 實體 HDC1080 模組）
- Yocto Phase：升級 QEMU 至 8.x 版本，支援 raspi4b machine，完整驗證 I2C probe 流程